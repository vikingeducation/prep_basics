...
Information from the server is broken down into uniform sized packets, there are many different types and sizes of these packets but they all share one thing, that both the server and the client can agree on the size of these data packages. This is important because after your ISP routes the data to your machine it must then be reassembled into its original form, a task which is done by your browsing application.

*Contributed by an anonymous user, iterated by [Jacob Farenci](https://github.com/JFarenci)*
