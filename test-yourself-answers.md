...
#### What the upstream used for?

The upstream is used for keeping track of the original repo. When you fork and clone the repo, the master branch could be updated in the meantime you are working on your feature, and when you try to push back to the master-branch it will give you an error because the file has been update from another user. To fix this, we use the upstream to keep track of the original repo, downloading the modified file from another user, and then we can push our file back to the master-branch.
