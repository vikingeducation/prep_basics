...
Nashville has several programming meetup groups, including one specifically for Rails. The groups seem to be pretty active and I’m looking forward to meeting some of my future peers. 

*Contributed by [Michael Albonetti](https://github.com/MikeAlbo/)*
...
Developer-friendly text editors offer many features that are helpful to developers including syntax coloring and plugins that aid in the development process. I’m currently using Brackets and it not only has Git integration, code folding, and auto indentions but will check for JS syntax errors. These are just a few options available. The core software is also customizable and I could contribute to it’s development if I wanted (or knew how) to.  Word processing software also creates a lot of formatting information that’s beyond what appears on the screen.  
...
HTML is the framework for the the client, or user side of the web. It provides the browser with the structure and content that is viewed by the user. HTML can be written, as in a static web page, or generated by server code, as in a web app. 
