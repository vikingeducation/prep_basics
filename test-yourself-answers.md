...
Front-end developers focus on the direct user interaction, the data that is sent to the back-end and how the website looks. Back-end developers on the other hand, work with the data that is stored on the server and presented to the user as well as storing the data generated by the user. The back-end developers also need to 'wire' up the requests from the front-end to the back-end and vice versa with the help of different programing languages and frameworks.
Fullstack developers have the knowledge to work in both fields, so the fullstack developer can build a website from A-Z on his own.

*Contributed by [Yilen Pan](http://github.com/yilenpan)*
*Contributed by [Sven Garson](https://github.com/SvenGarson)*
...
A DNS(Domain Name System) server is a server that converts a human-readable domain name like 'google.com' to its associated ip address, when a request is made from the browser.
So basically the DNS functions like a phone book service, where:
	- the 'domain name' is the name of a person
	- the 'ip address' is the telephone number associated with that person
...
Git is very useful for a developer because:
	- it allows the developer to 'roll back' to an older, working version of the project in case anything goes very wrong in the current version
	- the developer can save as many 'snapshots' of the projects as needed
	- one or multiple branches can exist in parallel and worked on independently, so as to make the development process very flexible in many ways
	- it allows development alone as well as in huge teams, and also supports multiple developers working on the same file(s) locally at the same time
	- provides great tools to merge changes together in an controlled and authored manner
